...
def test_next_p_and_pred_p():
    ...
    for p_val in test_values:
        ...
        assert pred_val == p_val, f"pred_p(next_p({p_val})) = {pred_val}, expected {p_val}"
        
        # Test reverse
        pred_val = pred_p(p_val)
        next_val = next_p(pred_val)
        assert next_val == p_val, f"next_p(pred_p({p_val})) = {next_val}, expected {p_val}"
...
def test_compatibility_with_original():
    ...
    try:
        import sys
        import os
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../collatz-private/src'))
        ...
        for p_val in test_values:
            ...
            assert binary_string(p_val) == orig_bits.binary(p_val), f"binary_string({p_val}) mismatch"
